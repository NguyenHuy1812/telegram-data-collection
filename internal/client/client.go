...
func Run(ctx context.Context) error {
	...
	options := telegram.Options{
		Logger:         nil,             // Passing logger for observability.
	...
	resolver := storage.NewResolverCache(peer.Plain(api), nil)
	_ = resolver

	// Create message handler
	messageHandler := usecase.NewMessageHandler(peerDB)

	// Registering handler for new channel messages
	...
	return waiter.Run(ctx, func(ctx context.Context) error {
		...
		if err := client.Run(ctx, func(ctx context.Context) error {
			...
			if arg.FillPeerStorage {
				...
				collector := storage.CollectPeers(nil)
